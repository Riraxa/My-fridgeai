...
export async function POST(req: NextRequest) {
  try {
    ...
    const ocrResult = await extractTextFromImageWithFallback(base64, file.type);
    ...
    // Check if AI processing failed (all items have confidence 0)
    if (parsedItems.length === 0 && allParsedItems.length > 0) {
      console.warn("AI processing may have failed - all items filtered out");
      // Still proceed with empty results rather than failing completely
    }
      ...
      for (const item of parsedItems) {
        const created = await tx.receiptItem.create({
          data: {
            ...
            estimatedExpirationDays: (item as any).estimated_expiration_days,
            processedCategory: (item as any).category || item.processedCategory,
