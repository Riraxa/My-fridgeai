...
type Tab = "basic" | "safety" | "ingredients" | "taste" | "lifestyle" | "genre" | "pro";
...
export default function EnhancedPreferencesPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [prefsRes, safetyRes, tasteRes, userRes, implicitRes] = await Promise.all([
  ...
  // カスタム暗黙食材の追加（ローカル状態のみ更新、保存はuseAutoSaveが担当）
  ...
  // カスタム暗黙食材の削除（ローカル状態のみ更新、保存はuseAutoSaveが担当）
  ...
  const {
    ...
    !readyToSave ? "" : JSON.stringify({ preferences, taste, genrePenalty, aiMessageEnabled, customImplicitIngredients }),
  ...
  const handleSafetyAutoSave = useCallback(
    async (value: string, signal: AbortSignal) => {
      ...
      // 現状のallergies/restrictionsをサーバーと同期
      // 差分を検出して個別に追加/削除
      ...
      // 削除: サーバーにあるが新しいデータにないもの
      const toDeleteAllergy = currentAllergies.filter(
        (a: typeof currentAllergies[0]) => !newAllergies.find((na: typeof newAllergies[0]) => na.id === a.id)
      );
      const toDeleteRestriction = currentRestrictions.filter(
        (r: typeof currentRestrictions[0]) => !newRestrictions.find((nr: typeof newRestrictions[0]) => nr.id === r.id)
      );
      
      // 追加: 新しいデータにIDがないもの
      const toAddAllergy = newAllergies.filter((na: typeof newAllergies[0]) => !na.id);
      const toAddRestriction = newRestrictions.filter((nr: typeof newRestrictions[0]) => !nr.id);
      
      await Promise.all([
        ...toDeleteAllergy.map((a: typeof toDeleteAllergy[0]) =>
          fetch(`/api/preferences/safety/${a.id}`, { method: "DELETE", signal })
        ),
        ...toDeleteRestriction.map((r: typeof toDeleteRestriction[0]) =>
          fetch(`/api/preferences/safety/${r.id}`, { method: "DELETE", signal })
        ),
        ...toAddAllergy.map((a: typeof toAddAllergy[0]) =>
          fetch("/api/preferences/safety", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
        ...
        ...toAddRestriction.map((r: typeof toAddRestriction[0]) =>
          fetch("/api/preferences/safety", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
                  ...
                  {cookware.map((c) => (
                    <button
                      ...
                      onClick={() => {
                        ...
                        setPreferences({
                          ...preferences,
                          kitchenCookware: list,
                        });
                      }}
                      className={`px-4 py-2 rounded-full text-xs font-semibold border transition ${(preferences.kitchenCookware ?? []).includes(c)
          ...
          {activeTab === "safety" && (
            <div className="space-y-6">
              ...
              >
                ...
                <p className="text-[10px] text-[var(--color-text-muted)] mt-3 px-1">
            ...
            <div className="space-y-6">
              ...
              >
                ...
                <p className="text-xs text-[var(--color-text-muted)] mb-4 px-1">
                  選択した食材は在庫登録なしでも常に使用可能です。献立生成時に不足として表示されません。
                </p>
                <div className="flex flex-wrap gap-2">
              ...
              >
                {!isPro && (
                  <div className="absolute inset-0 bg-[var(--background)]/60 backdrop-blur-[2px] z-10 flex flex-col items-center justify-center p-8 text-center">
                    ...
                    <p className="text-sm text-slate-500 mb-6">
                      Proプランで自分好みの基本食材を追加できます。卵、バター、こしょうなど、よく使う食材を追加して献立生成を快適に。
                    </p>
                    <button
                      onClick={() => router.push("/settings/account")}
                ...
                <div inert={!isPro || undefined} className={!isPro ? "pointer-events-none" : ""}>
                  ...
                  <p className="text-xs text-[var(--color-text-muted)] mb-4 px-1">
                    よく使う食材を追加すると、献立生成時に在庫チェックから除外されます。
                  </p>
                  ...
                  <div className="mb-6 flex flex-wrap gap-2">
                    {customImplicitIngredients.map((ing) => (
                      <div
                        key={ing}
                        className="bg-green-50 border border-green-100 px-3 py-1.5 rounded-full flex items-center gap-2 text-green-700 text-sm font-medium"
                      >
                        {ing}
                        <button
                          onClick={() => removeCustomImplicitIngredient(ing)}
                    ...
                    {customImplicitIngredients.length === 0 && (
                  ...
                  <div className="relative">
                    <input
                      type="text"
                      placeholder="追加する食材名を入力（例：卵、バター、こしょう）"
                  ...
                  <p className="text-[10px] text-[var(--color-text-muted)] mt-3 px-1">
                    ※最大20文字まで。重複は自動で除外されます。
                  </p>
                </div>
              </section>

              {/* Status Indicator - 基本食材タブ全体で共通 */}
              <div className="flex justify-start px-2 mt-4">
                {renderGlobalSaveIndicator(globalSaveState, globalSaveError)}
              </div>
            </div>
                ...
                >
                  ...
                  <div className="space-y-6">
                        ...
                        {(["short", "normal", "long"] as const).map((p) => (
                          <button
                            ...
                            className={`px-3 py-1.5 rounded-lg text-xs font-bold transition ${(taste.lifestyle[mode as keyof typeof taste.lifestyle] ?? {})
                              ?.timePriority === p
                              ? "bg-[var(--background)] text-indigo-600 shadow-sm"
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <Toggle
                        checked={(taste.lifestyle[mode as keyof typeof taste.lifestyle] ?? {}).dishwashingAvoid ?? false}
                        onChange={() =>
                          handleTasteChange({
                            lifestyle: {
                              ...(taste.lifestyle ?? {}),
  ...
  return (
    <div className="space-y-6">
      ...
      >
        {!isPro && (
          <div className="absolute inset-0 bg-[var(--background)]/60 backdrop-blur-[2px] z-10 flex flex-col items-center justify-center p-8 text-center">
            <div className="bg-[var(--semantic-indigo)] text-[#ffffff] p-4 rounded-3xl mb-4 shadow-xl shadow-indigo-100">
            ...
            <button
              onClick={() => router.push("/settings/account")}
              className="bg-[var(--semantic-indigo)] text-[#ffffff] px-8 py-3 rounded-2xl font-bold shadow-lg shadow-indigo-100 hover:scale-105 active:scale-95 transition"
            >
              Proにアップグレード
            </button>
          </div>
        ...
        <div inert={!isPro || undefined} className={!isPro ? "pointer-events-none" : ""}>
          ...
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex items-center gap-2">
              ...
              <Toggle
                ...
                onChange={() => handleToggle(!aiMessageEnabled)}
              />
            </div>
          </div>
          ...
          <div className="flex justify-between items-center mt-2 px-1">
            <div className="flex items-center gap-3">
              <span className="text-[10px] text-[var(--color-text-muted)]">※最大300文字</span>
              {renderSaveIndicator(saveState, errorMessage)}
            </div>
            <span
              className={`text-[10px] font-bold ${freeText.length > 280 ? "text-red-500" : "text-[var(--color-text-muted)]"
                }`}
            >
              {freeText.length}/300
            </span>
